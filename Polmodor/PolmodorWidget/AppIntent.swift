...
struct OpenAppIntent: AppIntent {
    static var title: LocalizedStringResource = "Open Polmodor"
    static var description: LocalizedStringResource = "Open the main Polmodor app"
    static var openAppWhenRun: Bool = true

    @MainActor
    func perform() async throws -> some IntentResult {
        // Notify the app to sync with LiveActivity state
        NotificationCenter.default.post(
            name: Notification.Name("SyncWithLiveActivity"), object: nil
        )
        return .result()
    }
}
